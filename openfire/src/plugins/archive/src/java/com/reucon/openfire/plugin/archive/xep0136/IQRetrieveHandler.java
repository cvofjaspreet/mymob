...
public class IQRetrieveHandler extends AbstractIQHandler
{
	...
	private static Logger Log = Logger.getLogger(IQRetrieveHandler.class);
    ...
    private Element addMessageElement(Element parentElement, Conversation conversation, ArchivedMessage message)
    {
      ...
      /*  String messageid=message.getSubject();
        if(messageid!=null){
        messageElement.addElement("messageid").setText(message.getSubject());
        }
        messageElement.addElement("type").setText(message.getType());
        messageElement.addElement("withjid").setText(message.getConversation().getWithJid());
        Log.info("elemet="+messageElement.toString());*/
