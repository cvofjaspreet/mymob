...
public class ArchiveManagerImpl implements ArchiveManager
{
    ...
    public void archiveMessage(Session session, Message message, boolean incoming)
    {
     ...
     // TODO support groupchat
        ...
        if(message.getBody()==null ){
        	return;
        }else if( message.getBody().equals("This room is not anonymous.")){
        	return;
        }else if(message.getTo()==null){
        	return;
        }else if( message.getTo().toBareJID().contains("hunter@mara.com")){
        	return;
        }
    ...
    private Conversation determineConversation(JID ownerJid, JID withJid, String subject, String thread, ArchivedMessage archivedMessage)
    {
        ...
        synchronized (activeConversations)
        {
            for (Conversation c : activeConversations)
            {
                ...
                if(archivedMessage.getType().equals("chat")){
                if (matches(ownerJid, withJid, thread, c))
                {
                    ...
                    break;
                	  ...
                	  if (matchesGroup(ownerJid, withJid, thread, c))
                      {
                          conversation = c;
                          break;
                      }
            ...
            if (conversation == null)
            {
                ...
                conversation = new Conversation(archivedMessage.getTime(),
                	  ...
                	  conversation = new Conversation(archivedMessage.getTime(),
                              ownerJid.toBareJID(), ownerJid.getResource(), withJid.toFullJID(), withJid.getResource(),
                              subject, thread);
                }
    ...
    private boolean matchesGroup(JID ownerJid, JID withJid, String thread, Conversation c)
    {
        if (! ownerJid.toBareJID().equals(c.getOwnerJid()))
        {
            return false;
        }
        if (! withJid.toFullJID().equals(c.getWithJid()))
        {
            return false;
        }
